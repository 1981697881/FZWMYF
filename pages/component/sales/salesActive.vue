		...
		<view class="box getheight">
			...
			<view class="cu-bar search bg-white" style="height: 60upx;">
				<view class="search-form round">
					...
					<input :adjust-position="false" type="text" :value="keyword" @input="inputChange" placeholder="搜索"
				...
				<view class="cu-list menu-avatar">
					<view class="cu-item" style="width: 100%;margin-top: 2px;height: 260upx;">
						<view style="clear: both;width: 100%;" class="grid text-left col-2"
							...
							<view class="text-grey" style="width: 100%;">生产单号:{{item.FPrdBillNo}}</view>
		...
		methods: {
			...
			showList(index, item) {
				...
				uni.navigateTo({
					...
					url: '../sales/salesPassive?billNo=' + item.FBillNo + '&tranType=' + this.source +
						'&type=2&startDate=' + this.start + '&endDate=' + this.end + '&FDeptNumber=' + item
						.FDeptNumber + '&FCustNumber=' + item.FCustNumber + '&FCustName=' + item.FCustName
				});
			},
			fetchData(val = '') {
				const me = this
